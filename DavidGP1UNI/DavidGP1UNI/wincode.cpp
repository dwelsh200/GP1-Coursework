...
ErrorType CreateDXWindow(char* title, int width, int height, int bits, bool fullscreenflag);
				   ...
				   int nCmdShow)				// Integer that tells how the window is to appear. Active/maximized, etc
{
	...
	if(CreateDXWindow("Shell Engine", 800, 600, 32, bFullScreen) == SUCCESS)
	{
		// The event loop
...
ErrorType CreateDXWindow(char* title, int width, int height, int bits, bool fullscreenflag)
{
	...
	{
		ErrorLogger::Writeln("Failed to register the window class.");
		return FAILURE;
	}
								...
								NULL)))								// Dont Pass Anything To WM_CREATE
	{
		...
		ErrorLogger::Writeln("Failed to create the window.");
		return FAILURE;
	}
